...
bool search (int tid, int2 coords, __constant float *calib, __constant float *Pose, __read_only image2d_t VMap, __read_only image2d_t NMap,
		...
		for (int k = 0; k < 27; k++) {			
			pt_T[k] = convert_double(nprev.x*pt_TB[k][0] + nprev.y*pt_TB[k][1] + nprev.z*pt_TB[k][2]);
		}
				
		pt_T[27] = -convert_double(nprev.x * (pt_ref[0] - vprev.x) + 
									nprev.y * (pt_ref[1] - vprev.y) +
									nprev.z * (pt_ref[2] - vprev.z));
				
		return true;

}

__kernel void SolveInitPbKernel(__read_only image2d_t VMap, __read_only image2d_t NMap,  
    ...
    if ((i*fact) < n_bump && (j*fact) < m_bump)
        found_coresp = search ((i*fact) * m_bump + (j*fact), (int2){j*fact,i*fact}, calib, Pose, VMap, NMap, VerticesBS, BumpSwap, BlendshapeCoeff, n_row, m_col, n_bump, m_bump, row);
