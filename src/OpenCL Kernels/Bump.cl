...
__kernel void BumpKernel(__global float *VMap, __global float *NMap, __global unsigned char *RGBMap, __global unsigned char *Flag, __global float *Bump, __global float *Mask,
	...
	for (int sbp = 0; sbp < MESSAGE_LENGTH; sbp++) {
		...
		p_indx[0] = min(m_bump - 1, max(0, Myround((pos[0] / fabs(pos[2]))*calib[0] + calib[2])));
		p_indx[1] = min(n_bump - 1, max(0, Myround((pos[1] / fabs(pos[2]))*calib[1] + calib[3])));
		
		int lb_i = max((n - p_indx[1] - 1) - 0, 0);
		int ub_i = min((n - p_indx[1] - 1) + 1, n);

		int lb_j = max(p_indx[0] - 0, 0);
		int ub_j = min(p_indx[0] + 1, m);
