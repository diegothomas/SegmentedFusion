...
__kernel void BTBKernel (__global float *buf, __global float *output, __constant int *Table_i, __constant int *Table_j, int nb_lines, int length, int length_out) {
 // Assume buf is of size group_size(0)*length
	...
	if (2*tid + b*STRIDE < length)
		sum = buf[nb_lines*(2*tid + b*STRIDE) + i] * buf[nb_lines*(2*tid + b*STRIDE) + j];
	if (2*tid + 1 + b*STRIDE < length)
		sum += buf[nb_lines*(2*tid  + 1 + b*STRIDE) + i] * buf[nb_lines*(2*tid + 1 + b*STRIDE) + j];
